...
export const WhiteboardCanvas: React.FC<WhiteboardCanvasProps> = ({
  ...
  const [resizeStart, setResizeStart] = useState<{ x: number; y: number; width: number; height: number } | null>(
    null
  );
  ...
  const handlePointerMove = (e: React.PointerEvent) => {
    ...
    // If resizing a sticky note
    if (resizingElementId && resizeStart && canEdit) {
      const dx = e.clientX - resizeStart.x;
      const dy = e.clientY - resizeStart.y;
      const newWidth = Math.max(90, Math.round(resizeStart.width + dx));
      const newHeight = Math.max(70, Math.round(resizeStart.height + dy));
      socketService.updateElement(roomId, resizingElementId, { width: newWidth, height: newHeight });
      return;
    }
  ...
  const getElementAtPosition = (pt: Point): WhiteboardElement | null => {
    ...
    for (let i = pageElements.length - 1; i >= 0; i--) {
      ...
      } else if (el.type === 'stroke' && el.points && el.points.length >= 2) {
        // Distance to each segment between consecutive points (not just the sampled points)
        const tolerance = Math.max(15, el.strokeWidth * 1.5);
        for (let j = 0; j < el.points.length - 1; j++) {
          const dist = distToSegment(pt, el.points[j], el.points[j + 1]);
          if (dist < tolerance) return el;
        }
      } else if (el.type === 'stroke' && el.points && el.points.length === 1) {
        const dist = Math.hypot(el.points[0].x - pt.x, el.points[0].y - pt.y);
          ...
          if (el.type === 'sticky') {
            return (
              ...
              >
                ...
                {isSelected && canEdit && (!el.isLocked || isTeacher) && (
                  <div
                    onPointerDown={(e) => {
                      e.stopPropagation();
                      try {
                    ...
                    className="absolute h-4 w-4 cursor-se-resize rounded-tl-md border border-white bg-blue-600"
                    style={{ right: -2, bottom: -2 }}
                    title="Arrastra para agrandar"
                  />
                )}
              </div>
            );
